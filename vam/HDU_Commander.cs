// HDU_Commander_v032_cloth_slider_zero_scan_wearall.cs
// v032_cloth_slider_zero_scan_wearall 2026-07-06
...
// v025: Replaces Target Cycle Pose/Back/Reset UI buttons with a Target Cycle Slider that drives target humanPoseControler's Cycle Pose Slider.
// v026: Replaces Self/Target Cloth SCAN/NEXT/PREV UI buttons with Self/Target Cloth State Sliders that drive ClothStateSwitcher v034+ Cloth State Slider.
// v027: Removes the Self/Target Cloth Slider Status text fields from the HDU UI; cloth control remains slider-only.
// v028: Cloth sliders allow -0.01 as a reload trigger and reset to 0 after reload.
// v029: Colors slider backgrounds/tracks instead of slider label text for Cloth and Target Cycle sliders.
// v030: Cloth sliders read ClothStateSwitcher v035 Max/Current/Target/Applying, debounce HDU writes, and stop syncing while the user is dragging so the thumb does not catch/stutter.
// v031: -0.01 cloth slider trigger calls Scan Cloth And Wear All and returns the HDU slider to 0.
// v032: Removes the -0.01 UI position. Cloth slider min is 0, and user-moving the slider to 0 triggers Scan Cloth And Wear All.
...
public class HDU_Commander_v032_cloth_slider_zero_scan_wearall : MVRScript
{
    private const string VERSION = "v032_cloth_slider_zero_scan_wearall";
    private const string ANY = "ANY";
    private const string NONE = "None";
    private const float CLOTH_SLIDER_DEFAULT_MAX = 50.0f;
    private const float CLOTH_SLIDER_APPLY_DELAY_SEC = 0.12f;
    private const float CLOTH_SLIDER_LOCAL_SYNC_GRACE_SEC = 0.70f;
    private const float CLOTH_SLIDER_ZERO_RELOAD_EPS = 0.0001f;
    ...
    private JSONStorableFloat targetCycleSliderJSON;
    private JSONStorableString targetCycleSliderStatusJSON;
    private JSONStorableFloat selfClothSliderJSON;
    private JSONStorableFloat targetClothSliderJSON;
    private UIDynamicSlider selfClothSliderUI;
    private UIDynamicSlider targetClothSliderUI;
    private bool selfClothSliderSuppressApply = false;
    private bool targetClothSliderSuppressApply = false;
    private bool selfClothSliderPendingApply = false;
    private bool targetClothSliderPendingApply = false;
    private float selfClothSliderPendingValue = 0.0f;
    private float targetClothSliderPendingValue = 0.0f;
    private float selfClothSliderLastLocalChangeTime = -999.0f;
    private float targetClothSliderLastLocalChangeTime = -999.0f;
    ...
    public void Update()
    {
        UpdatePendingClothSliderApplies();
    ...
    private void BuildUi()
    {
        ...
        // v026: Cloth controls are slider-only in HDU.
        // ClothStateSwitcher itself still owns scan/order/PHTY logic; HDU only drives its registered Cloth State Slider.
        CreateClothSliderControlsLeft();
        ...
        targetCycleSliderJSON = new JSONStorableFloat("Target Cycle Slider", 0.0f, 0.0f, 27.0f, true, true);
        targetCycleSliderJSON.setCallbackFunction = OnTargetCycleSliderChanged;
        RegisterFloat(targetCycleSliderJSON);
        ApplySliderBackgroundColor(CreateSlider(targetCycleSliderJSON, true), TargetButtonColor());

        targetCycleSliderStatusJSON = new JSONStorableString("Target Cycle Slider Status", "Target Cycle Slider: Ready / 0.00");
        RegisterString(targetCycleSliderStatusJSON);
        UIDynamicTextField targetCycleField = CreateTextField(targetCycleSliderStatusJSON, true);
        if (targetCycleField != null)
            targetCycleField.height = 42.0f;
    }
    ...
    private void RegisterExternalActions()
    {
        ...
        // Compatibility actions remain, but the HDU UI is slider-only for cloth in v026.
        ...
        RegisterAction(new JSONStorableAction("Self Cloth Slider Apply", delegate
        {
            ApplyClothStateSlider(false, selfClothSliderJSON != null ? selfClothSliderJSON.val : 0.0f, "Self Cloth Slider Apply");
        }));
        RegisterAction(new JSONStorableAction("Target Cloth Slider Apply", delegate
        {
            ApplyClothStateSlider(true, targetClothSliderJSON != null ? targetClothSliderJSON.val : 0.0f, "Target Cloth Slider Apply");
        }));
        RegisterAction(new JSONStorableAction("Self Cloth Slider Reload", delegate
        {
            ReloadClothStateFromSlider(false, "Self Cloth Slider");
        }));
        RegisterAction(new JSONStorableAction("Target Cloth Slider Reload", delegate
        {
            ReloadClothStateFromSlider(true, "Target Cloth Slider");
        }));
        ...
        RegisterAction(new JSONStorableAction("Target Cycle Slider Apply", delegate
        {
            ApplyTargetCycleSlider(targetCycleSliderJSON != null ? targetCycleSliderJSON.val : 0.0f, "Target Cycle Slider Apply");
        }));
    }
    ...
    private void ApplySliderBackgroundColor(Component dynamicSlider, Color baseColor)
    {
        ...
        try
        {
            // v029: Color the slider UI background/track, not the label text.
            // VaM slider prefab names can vary, so prefer Image components and leave Text untouched.
            Image[] images = dynamicSlider.GetComponentsInChildren<Image>(true);
            if (images == null)
                return;

            Color background = Color.Lerp(baseColor, Color.white, 0.25f);
            background.a = 0.85f;
            Color track = Color.Lerp(baseColor, Color.white, 0.05f);
            track.a = 0.95f;
            ...
            for (int i = 0; i < images.Length; i++)
            {
                ...
                if (n.IndexOf("background") >= 0 || n.IndexOf("bg") >= 0)
                {
                    img.color = background;
                    continue;
                }

                if (n.IndexOf("fill") >= 0 || n.IndexOf("handle") >= 0)
                {
                    img.color = track;
                    continue;
                }

                // Fallback: color row/panel images too, but do not touch label Text components.
                img.color = background;
            }
        }
        catch
    ...
    private void OnTargetCycleSliderChanged(float value)
    {
        ApplyTargetCycleSlider(value, "Target Cycle Slider");
    }

    private bool ApplyTargetCycleSlider(float value, string label)
    {
        PluginHit hit;
        if (!TryFindTargetHumanControler(out hit) || hit == null || hit.storable == null)
        {
            UpdateTargetCycleSliderStatus("missing target humanControler", value);
        ...
        JSONStorableFloat targetSlider = hit.storable.GetFloatJSONParam("Cycle Pose Slider");
        if (targetSlider == null)
        {
            UpdateTargetCycleSliderStatus("missing Cycle Pose Slider", value);
            SetStatus("Missing float: " + label + " / target HC Cycle Pose Slider");
            return false;
        }

        targetSlider.val = value;
        UpdateTargetCycleSliderStatus("set", value);
        SetStatus("OK: " + label + " -> " + hit.atom.uid + " / " + hit.storableId + " / Cycle Pose Slider=" + value.ToString("F2"));
        DebugLog("set target HC slider / label=" + label + " / atom=" + hit.atom.uid + " / storable=" + hit.storableId + " / value=" + value.ToString("F2"));
        return true;
    }

    private void UpdateTargetCycleSliderStatus(string state, float value)
    {
        if (targetCycleSliderStatusJSON != null)
            targetCycleSliderStatusJSON.val = "Target Cycle Slider: " + state + " / " + value.ToString("F2");
    }

    private void CreateClothSliderControlsLeft()
    {
        // v032: slider min is 0. User-moving to 0 means SCAN + WEAR ALL.
        selfClothSliderJSON = new JSONStorableFloat("Self Cloth Slider", 0.0f, 0.0f, 50.0f, true, true);
        selfClothSliderJSON.setCallbackFunction = OnSelfClothSliderChanged;
        RegisterFloat(selfClothSliderJSON);
        selfClothSliderUI = CreateSlider(selfClothSliderJSON, false);
        ApplySliderBackgroundColor(selfClothSliderUI, SelfButtonColor());
        ApplyClothSliderUiRange(false, 50.0f);

        targetClothSliderJSON = new JSONStorableFloat("Target Cloth Slider", 0.0f, 0.0f, 50.0f, true, true);
        targetClothSliderJSON.setCallbackFunction = OnTargetClothSliderChanged;
        RegisterFloat(targetClothSliderJSON);
        targetClothSliderUI = CreateSlider(targetClothSliderJSON, false);
        ApplySliderBackgroundColor(targetClothSliderUI, TargetButtonColor());
        ApplyClothSliderUiRange(true, 50.0f);
    }

    private void OnSelfClothSliderChanged(float value)
    {
        if (selfClothSliderSuppressApply)
            return;

        QueueClothStateSliderApply(false, value, "Self Cloth Slider");
    }

    private void OnTargetClothSliderChanged(float value)
    {
        if (targetClothSliderSuppressApply)
            return;

        QueueClothStateSliderApply(true, value, "Target Cloth Slider");
    }

    private void QueueClothStateSliderApply(bool targetSide, float value, string label)
    {
        // v032: user-moving the HDU cloth slider to 0 means "SCAN + WEAR ALL".
        // This should be immediate and should not leave a pending slider write.
        if (value <= CLOTH_SLIDER_ZERO_RELOAD_EPS)
        {
            ClearPendingClothSliderApply(targetSide);
            ReloadClothStateFromSlider(targetSide, label);
            return;
        }

        if (targetSide)
        {
            targetClothSliderPendingApply = true;
            targetClothSliderPendingValue = value;
            targetClothSliderLastLocalChangeTime = Time.time;
        }
        else
        {
            selfClothSliderPendingApply = true;
            selfClothSliderPendingValue = value;
            selfClothSliderLastLocalChangeTime = Time.time;
        }

        DebugLog((targetSide ? "Target" : "Self") + " Cloth Slider queued / value=" + value.ToString("F2"));
    }

    private void ClearPendingClothSliderApply(bool targetSide)
    {
        if (targetSide)
            targetClothSliderPendingApply = false;
        else
            selfClothSliderPendingApply = false;
    }

    private void UpdatePendingClothSliderApplies()
    {
        if (selfClothSliderPendingApply && Time.time - selfClothSliderLastLocalChangeTime >= CLOTH_SLIDER_APPLY_DELAY_SEC)
        {
            selfClothSliderPendingApply = false;
            float value = Mathf.Round(selfClothSliderPendingValue);
            SetLocalClothSliderValue(false, value);
            ApplyClothStateSlider(false, value, "Self Cloth Slider");
        }

        if (targetClothSliderPendingApply && Time.time - targetClothSliderLastLocalChangeTime >= CLOTH_SLIDER_APPLY_DELAY_SEC)
        {
            targetClothSliderPendingApply = false;
            float value = Mathf.Round(targetClothSliderPendingValue);
            SetLocalClothSliderValue(true, value);
            ApplyClothStateSlider(true, value, "Target Cloth Slider");
        }
    }

    private bool IsLocalClothSliderActive(bool targetSide)
    {
        if (targetSide)
            return targetClothSliderPendingApply || (Time.time - targetClothSliderLastLocalChangeTime < CLOTH_SLIDER_LOCAL_SYNC_GRACE_SEC);
        return selfClothSliderPendingApply || (Time.time - selfClothSliderLastLocalChangeTime < CLOTH_SLIDER_LOCAL_SYNC_GRACE_SEC);
    }

    private bool ApplyClothStateSlider(bool targetSide, float value, string label)
    {
        PluginHit hit;
        ...
        {
            UpdateClothSliderStatus(targetSide, "missing ClothStateSwitcher", value, "");
        ...
        JSONStorableFloat remoteSlider = hit.storable.GetFloatJSONParam("Cloth State Slider");
        if (remoteSlider == null)
        {
            UpdateClothSliderStatus(targetSide, "missing Cloth State Slider", value, BuildClothProgressLabel(hit));
            SetStatus("Missing float: " + label + " / Cloth State Slider");
            return false;
        }

        float max = GetRemoteClothSliderMax(hit, remoteSlider);
        ApplyLocalClothSliderRange(targetSide, max);
        float target = Mathf.Clamp(Mathf.Round(value), 0.0f, max);
        SetLocalClothSliderValue(targetSide, target);

        remoteSlider.val = target;
        UpdateClothSliderStatus(targetSide, "set", target, BuildClothProgressLabel(hit));
        SetStatus("OK: " + label + " -> " + hit.atom.uid + " / " + hit.storableId + " / Cloth State Slider=" + target.ToString("F2") + " / max=" + max.ToString("F0"));
        DebugLog("set cloth slider / label=" + label + " / atom=" + hit.atom.uid + " / storable=" + hit.storableId + " / value=" + target.ToString("F2") + " / max=" + max.ToString("F0"));
        return true;
    }

    private bool ReloadClothStateFromSlider(bool targetSide, string label)
    {
        PluginHit hit;
        bool found = targetSide ? TryFindTargetClothSwitcher(out hit) : TryFindSelfClothSwitcher(out hit);
        if (!found || hit == null || hit.storable == null)
        {
            ResetLocalClothSliderToZero(targetSide);
            SetStatus("Missing: " + label + " reload / ClothStateSwitcher " + (targetSide ? "target" : "self"));
            return false;
        }

        EnsureClothSwitcherSelfSelection(hit, targetSide ? "target" : "self");

        // v032: slider value 0 means SCAN + WEAR ALL. Prefer the combined action from ClothStateSwitcher v036+.
        bool ok = TriggerActionOnHit(hit,
            new string[] { "Scan Cloth And Wear All", "Cloth Slider Scan And Wear All", "Scan And Wear All" },
            label + " Scan+WearAll",
            "cloth reload");

        // Fallback for older ClothStateSwitcher builds: restore current list first, then scan/refresh.
        if (!ok)
        {
            bool wearOk = TriggerActionOnHit(hit,
                new string[] { "Restore All", "Cloth Slider Wear All" },
                label + " WearAll fallback",
                "cloth reload");

            bool scanOk = TriggerActionOnHit(hit,
                new string[] { "Scan Cloth", "Refresh Cloth State Slider" },
                label + " Scan fallback",
                "cloth reload");
        ...
        if (ok)
        {
            SetStatus("OK: " + label + " scan+wear all -> " + hit.atom.uid + " / " + hit.storableId);
            DebugLog("cloth slider scan+wear all / label=" + label + " / atom=" + hit.atom.uid + " / storable=" + hit.storableId);
        }
        else
        {
            SetStatus("Missing action: " + label + " reload / Scan Cloth And Wear All");
        }

        return ok;
    }

    private float GetRemoteClothSliderMax(PluginHit hit, JSONStorableFloat remoteSlider)
    {
        try
        {
            if (hit != null && hit.storable != null)
            {
                JSONStorableFloat maxParam = hit.storable.GetFloatJSONParam("Cloth State Slider Max");
                if (maxParam != null)
                    return Mathf.Max(0.0f, maxParam.val);
            }

            if (remoteSlider != null)
                return Mathf.Max(0.0f, remoteSlider.max);
        }
        catch { }

        return CLOTH_SLIDER_DEFAULT_MAX;
    }

    private float GetRemoteClothSliderDisplayValue(PluginHit hit, JSONStorableFloat remoteSlider)
    {
        float value = remoteSlider != null ? remoteSlider.val : 0.0f;
        try
        {
            if (hit != null && hit.storable != null)
            {
                JSONStorableBool applying = hit.storable.GetBoolJSONParam("Cloth State Slider Applying");
                JSONStorableFloat target = hit.storable.GetFloatJSONParam("Cloth State Slider Target");
                if (applying != null && applying.val && target != null)
                    value = target.val;
            }
        }
        catch { }
    ...
    private void ApplyLocalClothSliderRange(bool targetSide, float max)
    {
        JSONStorableFloat slider = targetSide ? targetClothSliderJSON : selfClothSliderJSON;
        if (slider == null)
            return;
        ...
        {
            slider.min = 0.0f;
            slider.max = Mathf.Max(1.0f, max);
        }
        catch { }

        ApplyClothSliderUiRange(targetSide, max);
    }

    private void ApplyClothSliderUiRange(bool targetSide, float max)
    {
        UIDynamicSlider ui = targetSide ? targetClothSliderUI : selfClothSliderUI;
        if (ui == null)
            return;
        ...
        try
        {
            if (ui.slider != null)
            {
                ui.slider.minValue = 0.0f;
                ui.slider.maxValue = Mathf.Max(1.0f, max);
            }
        }
        catch { }
    }

    private void SetLocalClothSliderValue(bool targetSide, float value)
    {
        if (targetSide)
        {
            targetClothSliderSuppressApply = true;
            try { if (targetClothSliderJSON != null) targetClothSliderJSON.val = value; } catch { }
            targetClothSliderSuppressApply = false;
        }
        else
        {
            selfClothSliderSuppressApply = true;
            try { if (selfClothSliderJSON != null) selfClothSliderJSON.val = value; } catch { }
            selfClothSliderSuppressApply = false;
        }
    }

    private void ResetLocalClothSliderToZero(bool targetSide)
    {
        ClearPendingClothSliderApply(targetSide);
        if (targetSide)
            targetClothSliderLastLocalChangeTime = Time.time;
            ...
            selfClothSliderLastLocalChangeTime = Time.time;
        SetLocalClothSliderValue(targetSide, 0.0f);
    }

    private void UpdateClothSliderStates()
    {
        SyncOneClothSlider(false);
        SyncOneClothSlider(true);
    }

    private void SyncOneClothSlider(bool targetSide)
    {
        PluginHit hit;
        bool found = targetSide ? TryFindTargetClothSwitcher(out hit) : TryFindSelfClothSwitcher(out hit);
        if (!found || hit == null || hit.storable == null)
        {
            UpdateClothSliderStatus(targetSide, "missing", targetSide && targetClothSliderJSON != null ? targetClothSliderJSON.val : (selfClothSliderJSON != null ? selfClothSliderJSON.val : 0.0f), "");
            return;
        }
        ...
        if (remoteSlider == null)
        {
            UpdateClothSliderStatus(targetSide, "missing Cloth State Slider", 0.0f, BuildClothProgressLabel(hit));
            return;
        }
        ...
        float value = Mathf.Clamp(GetRemoteClothSliderDisplayValue(hit, remoteSlider), 0.0f, max);

        // v030: do not pull the HDU slider thumb back to the remote value while the user is dragging
        // or right after an HDU change. This avoids the sticky/catching feel.
        if (!IsLocalClothSliderActive(targetSide))
            SetLocalClothSliderValue(targetSide, value);

        string remoteStatus = "";
        JSONStorableString status = hit.storable.GetStringJSONParam("Cloth State Slider Status");
        if (status != null && !string.IsNullOrEmpty(status.val))
            remoteStatus = status.val;

        UpdateClothSliderStatus(targetSide, IsLocalClothSliderActive(targetSide) ? "local" : "sync", value, BuildClothProgressLabel(hit) + " / max=" + max.ToString("F0") + (string.IsNullOrEmpty(remoteStatus) ? "" : " / " + remoteStatus));
    }

    private string BuildClothProgressLabel(PluginHit hit)
    {
        int hidden;
        int total;
        if (TryGetClothProgress(hit, out hidden, out total) && total > 0)
            return hidden.ToString() + "/" + total.ToString();
        return "-/-";
    }

    private void UpdateClothSliderStatus(bool targetSide, string state, float value, string progress)
    {
        // v027: no visible cloth slider status fields in HDU.
        // Keep this method as a no-op so sync/apply call sites stay simple.
        DebugLog((targetSide ? "Target" : "Self") + " Cloth Slider " + state + " / value=" + value.ToString("F2") + (string.IsNullOrEmpty(progress) ? "" : " / " + progress));
    }
    ...
    {
        // v026: kept as a compatibility wrapper for old external action paths.
        // No Cloth buttons are created in the HDU UI anymore.
        UpdateClothSliderStates();
    }

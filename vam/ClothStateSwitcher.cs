// V38_CLOTH_SLIDER_DELTA_COMPILE_FIX: v36 base + Cloth State Slider PHTY FADE starts the target group immediately in the background, marks pending hidden for slider sync, and no longer waits like a queue.
// V36_CLOTH_SLIDER_SCAN_WEARALL: v35 base + adds combined Scan Cloth And Wear All action for HDU -0.01 slider reload.
// V35_CLOTH_SLIDER_EXTERNAL_SYNC: v34 base + exposes Cloth State Slider Max/Current/Target/Applying so HDU can sync slider range without fighting user drag.
// V34_CLOTH_SLIDER_RESPONSIVE_FADE: v33 base + Cloth State Slider keeps the thumb at the requested target while PHTY/FADE is running, so FADE no longer snaps/waits visually until completion.
// V33_CLOTH_SLIDER_GROUP_PHTY: v32 base + Cloth State Slider uses grouped PHTY for DOWN/UP/FADE, so moving several steps hides that group in one simultaneous physical/fade pass instead of waiting item-by-item. RIPPER remains guarded by ClothingRipper detection.
// V32_CLOTH_SLIDER_PHTY_RIPPER_GUARD: v31 base + Cloth State Slider now honors PHTY DOWN/UP/FADE/RIPPER by stepping sequentially; RIPPER is hidden from PHTY choices when ClothingRipper is not detected.
// V31_CLOTH_STATE_SLIDER: v30 base + adds Cloth State Slider. 0=all worn, max=all operable hidden; slider applies direct visual hide/wear state and keeps NEXT/PREV buttons.
...
public class ClothStateSwitcher : MVRScript
{
    ...
    private JSONStorableFloat clothStateSliderJSON;
    private JSONStorableString clothStateSliderStatusJSON;
    // v035: hidden external sync values for HDU/other control panels.
    private JSONStorableFloat clothStateSliderMaxJSON;
    private JSONStorableFloat clothStateSliderCurrentJSON;
    private JSONStorableFloat clothStateSliderTargetJSON;
    private JSONStorableBool clothStateSliderApplyingJSON;
    ...
    private bool clothStateSliderSuppressCallback = false;
    private int clothStateSliderLastTarget = -1;
    private Coroutine clothStateSliderApplyRoutine = null;
    private int clothStateSliderApplyVersion = 0;
    private bool clothStateSliderApplyInProgress = false;
    private int clothStateSliderPendingTarget = -1;
    // v037: PHTY FADE from the slider is fire-and-follow. The slider counts these as hidden
    // immediately so HDU/the local scrollbar do not wait for the visual fade coroutine to finish.
    private readonly HashSet<string> clothStateSliderPendingHiddenPrefixes = new HashSet<string>();
    ...
    private Coroutine scanClothAndWearAllRoutine = null;
    ...
    public override void Init()
    {
        ...
        physRemoveStyleChooser = new JSONStorableStringChooser(
            "PHTY",
            BuildPhysRemoveStyleChoices(false),
            PHYS_DOWN,
            "PHTY",
        ...
        clothStateSliderJSON = new JSONStorableFloat("Cloth State Slider", 0.0f, 0.0f, 50.0f, true, true);
        clothStateSliderJSON.setCallbackFunction = OnClothStateSliderChanged;
        RegisterFloat(clothStateSliderJSON);

        clothStateSliderStatusJSON = new JSONStorableString("Cloth State Slider Status", "Cloth Slider: no cloth list");
        RegisterString(clothStateSliderStatusJSON);

        // v035: these are registered but not shown in the UI. HDU reads them to set its own slider range
        // and avoid snapping the local HDU slider while FADE/PHTY is still applying.
        clothStateSliderMaxJSON = new JSONStorableFloat("Cloth State Slider Max", 0.0f, 0.0f, 200.0f, true, true);
        clothStateSliderCurrentJSON = new JSONStorableFloat("Cloth State Slider Current", 0.0f, 0.0f, 200.0f, true, true);
        clothStateSliderTargetJSON = new JSONStorableFloat("Cloth State Slider Target", 0.0f, 0.0f, 200.0f, true, true);
        clothStateSliderApplyingJSON = new JSONStorableBool("Cloth State Slider Applying", false);
        RegisterFloat(clothStateSliderMaxJSON);
        RegisterFloat(clothStateSliderCurrentJSON);
        RegisterFloat(clothStateSliderTargetJSON);
        RegisterBool(clothStateSliderApplyingJSON);
        ...
        CreateSlider(clothStateSliderJSON, true);
        UIDynamicTextField clothSliderStatusField = CreateTextField(clothStateSliderStatusJSON, true);
        if (clothSliderStatusField != null)
            clothSliderStatusField.height = 42.0f;
        ...
        UpdateButtonColors();
        DebugLog("ready / v32 cloth slider phty ripper guard / v29 global exclusion manual load button / path=Saves/PluginData/VAMT_ClothStateSwitcher_GlobalExclusions.json / no System.IO / no FileManagerSecure / no GetType");
    }
    ...
    private IEnumerator AppearancePresetResetRoutine()
    {
        ...
        clothStateSliderPendingHiddenPrefixes.Clear();
        stateCacheValid = false;
    ...
    private void ScanClothAndWearAll()
    {
        LogButtonPressed("SCAN + WEAR ALL");

        if (scanClothAndWearAllRoutine != null)
        {
            try { StopCoroutine(scanClothAndWearAllRoutine); } catch { }
            scanClothAndWearAllRoutine = null;
        }

        scanClothAndWearAllRoutine = StartCoroutine(ScanClothAndWearAllRoutine());
    }

    private IEnumerator ScanClothAndWearAllRoutine()
    {
        // First restore using the currently known cloth list so the visible state is safe before rescanning.
        ...
        RestoreAllVisible();
        while (restoreAllRoutine != null)
            yield return null;

        // Then rescan/reload the cloth list. RequestScanClothes already handles delayed material rebuilds and retries.
        RequestScanClothes();
        while (delayedScanRoutine != null)
            yield return null;

        // After the fresh scan, restore once more so newly detected items are also forced visible.
        RestoreAllVisible();
        while (restoreAllRoutine != null)
            yield return null;

        clothStateSliderApplyInProgress = false;
        clothStateSliderPendingTarget = -1;
        clothStateSliderLastTarget = 0;
        RefreshClothStateSliderRangeAndValue(true);

        if (clothStateSliderJSON != null)
        {
            clothStateSliderSuppressCallback = true;
            try { clothStateSliderJSON.val = 0.0f; } catch { }
            clothStateSliderSuppressCallback = false;
        }

        UpdateExternalState("scan_wear_all", null);
        UpdatePreview();
        SetStatus("Scan Cloth + Wear All done / accepted=" + lastScanAccepted.ToString(CultureInfo.InvariantCulture));
        scanClothAndWearAllRoutine = null;
    }
    ...
    private void RegisterExternalActions()
    {
        ...
        RegisterAction(new JSONStorableAction("Scan Cloth And Wear All", ScanClothAndWearAll));
        RegisterAction(new JSONStorableAction("Cloth Slider Scan And Wear All", ScanClothAndWearAll));
        RegisterAction(new JSONStorableAction("Scan And Wear All", ScanClothAndWearAll));
        ...
        RegisterAction(new JSONStorableAction("Refresh Cloth State Slider", delegate() { RefreshClothStateSliderRangeAndValue(true); }));
        RegisterAction(new JSONStorableAction("Refresh Cloth Slider Sync", delegate() { RefreshClothStateSliderRangeAndValue(true); }));
        RegisterAction(new JSONStorableAction("Cloth Slider Wear All", delegate() { SetClothStateSliderTarget(0); }));
        RegisterAction(new JSONStorableAction("Cloth Slider Hide All", delegate() { SetClothStateSliderTarget(GetClothStateSliderMax()); }));
    ...
    private List<string> BuildPhysRemoveStyleChoices(bool includeRipper)
    {
        List<string> choices = new List<string>();
        choices.Add(PHYS_OFF);
        choices.Add(PHYS_DOWN);
        choices.Add(PHYS_UP);
        choices.Add(PHYS_FADE);
        if (includeRipper)
            choices.Add(PHYS_RIPPER);
        return choices;
    }

    private void RefreshPhysRemoveStyleChoicesForRipper(bool ripperFound, string reason)
    {
        if (physRemoveStyleChooser == null)
            return;

        string before = physRemoveStyleChooser.val;
        List<string> choices = BuildPhysRemoveStyleChoices(ripperFound);
        physRemoveStyleChooser.choices = choices;

        bool beforeIsRipper = before == PHYS_RIPPER || before == PHYS_RIPPER_ALL || before == PHYS_RIPPER_SELECTED;
        if (!ripperFound && beforeIsRipper)
        {
            physRemoveStyleChooser.val = PHYS_DOWN;
            SetStatus("PHTY RIPPER hidden: ClothingRipper not found / switched to DOWN");
        }
        else if (!string.IsNullOrEmpty(before) && choices.Contains(before))
        {
            physRemoveStyleChooser.val = before;
        }
        else if (string.IsNullOrEmpty(physRemoveStyleChooser.val) || !choices.Contains(physRemoveStyleChooser.val))
        {
            physRemoveStyleChooser.val = PHYS_DOWN;
        }

        DebugLog("[PHTY CHOICES] reason=" + reason + " / ripper=" + (ripperFound ? "1" : "0") + " / before=" + before + " / now=" + physRemoveStyleChooser.val);
    }
    ...
    private void SetPhysMode(string mode)
    {
        ...
        if (mode == PHYS_RIPPER)
        {
            ...
            {
                physRemoveStyleChooser.val = PHYS_DOWN;
                SetStatus("Phys Remove Style: RIPPER unavailable / ClothingRipper not found / switched to DOWN");
                UpdatePreview();
                return;
            }

            physRemoveStyleChooser.val = PHYS_RIPPER;
            SetStatus("Phys Remove Style: RIPPER");
        }
        else
        {
    ...
    private void RebuildOrderOnly()
    {
        ...
        ApplyFinalRule();
        UpdateExclusionChoices();
        RefreshClothStateSliderRangeAndValue(false);
        UpdatePreview();
    }
    ...
    private int GetClothStateSliderMax()
    {
        int count = 0;
        for (int i = 0; i < orderedClothes.Count; i++)
        {
    ...
    private int GetClothStateSliderCurrentHidden()
    {
        int count = 0;
        for (int i = 0; i < orderedClothes.Count; i++)
        {
            ...
            if (IsProtected(item))
                continue;
            if (IsHidden(item) || IsClothSliderPendingHidden(item))
                count++;
        }
        return count;
    }

    private void RefreshClothStateSliderRangeAndValue(bool forceStatus)
    {
        if (clothStateSliderJSON == null)
            return;

        int max = Mathf.Max(0, GetClothStateSliderMax());
        int hidden = Mathf.Clamp(GetClothStateSliderCurrentHidden(), 0, max);

        // v034: while a PHTY/FADE slider operation is running, keep the UI thumb at the requested
        // target instead of snapping it back to the current hidden count. The real cloth state still
        // catches up after the fade/physical pass completes.
        int displayValue = hidden;
        if (clothStateSliderApplyInProgress && clothStateSliderPendingTarget >= 0)
            displayValue = Mathf.Clamp(clothStateSliderPendingTarget, 0, max);

        clothStateSliderSuppressCallback = true;
        try
        {
            try { clothStateSliderJSON.min = 0.0f; } catch { }
            try { clothStateSliderJSON.max = Mathf.Max(1.0f, (float)max); } catch { }
            clothStateSliderJSON.val = (float)displayValue;
        }
        catch { }
        clothStateSliderSuppressCallback = false;

        clothStateSliderLastTarget = displayValue;
        UpdateClothStateSliderExternalValues(hidden, max, displayValue);
        if (clothStateSliderApplyInProgress && displayValue != hidden)
            UpdateClothStateSliderStatus("applying", hidden, max, forceStatus);
        else
            UpdateClothStateSliderStatus("sync", hidden, max, forceStatus);
    }

    private void UpdateClothStateSliderStatus(string state, int hidden, int max, bool force)
    {
        if (clothStateSliderStatusJSON == null)
            return;

        string pending = "";
        if (clothStateSliderApplyInProgress && clothStateSliderPendingTarget >= 0)
            pending = " / target=" + Mathf.Clamp(clothStateSliderPendingTarget, 0, max).ToString(CultureInfo.InvariantCulture);

        clothStateSliderStatusJSON.val = "Cloth Slider: " + state + " / hidden=" +
            hidden.ToString(CultureInfo.InvariantCulture) + "/" + max.ToString(CultureInfo.InvariantCulture) +
            pending +
            " / 0=WEAR ALL / max=WEAR NONE";
    }

    private void UpdateClothStateSliderExternalValues(int hidden, int max, int target)
    {
        ...
        try
        {
            if (clothStateSliderMaxJSON != null)
            {
                try { clothStateSliderMaxJSON.max = Mathf.Max(1.0f, (float)max); } catch { }
                clothStateSliderMaxJSON.val = (float)max;
            }
            if (clothStateSliderCurrentJSON != null)
            {
                try { clothStateSliderCurrentJSON.max = Mathf.Max(1.0f, (float)max); } catch { }
                clothStateSliderCurrentJSON.val = (float)Mathf.Clamp(hidden, 0, max);
            }
            if (clothStateSliderTargetJSON != null)
            {
                try { clothStateSliderTargetJSON.max = Mathf.Max(1.0f, (float)max); } catch { }
                clothStateSliderTargetJSON.val = (float)target;
            }
            if (clothStateSliderApplyingJSON != null)
                clothStateSliderApplyingJSON.val = clothStateSliderApplyInProgress;
        }
        catch { }
    }

    private void OnClothStateSliderChanged(float value)
    {
        if (clothStateSliderSuppressCallback)
            return;

        int max = Mathf.Max(0, GetClothStateSliderMax());
        int target = Mathf.Clamp(Mathf.RoundToInt(value), 0, max);

        clothStateSliderSuppressCallback = true;
        try
        {
            if (clothStateSliderJSON != null)
                clothStateSliderJSON.val = (float)target;
        }
        catch { }
        clothStateSliderSuppressCallback = false;

        if (target == clothStateSliderLastTarget && GetClothStateSliderCurrentHidden() == target)
        {
            UpdateClothStateSliderStatus("ready", target, max, false);
            return;
        }

        SetClothStateSliderTarget(target);
    }

    private void SetClothStateSliderTarget(int targetHiddenCount)
    {
        int max = Mathf.Max(0, GetClothStateSliderMax());
        int target = Mathf.Clamp(targetHiddenCount, 0, max);

        if (clothStateSliderApplyRoutine != null)
        {
            clothStateSliderApplyVersion++;
            try { StopCoroutine(clothStateSliderApplyRoutine); } catch { }
            clothStateSliderApplyRoutine = null;
        }

        // v034: commit the slider UI to the requested target immediately. The actual clothing
        // visibility catches up asynchronously, but the scrollbar no longer waits for FADE to end.
        clothStateSliderApplyInProgress = true;
        clothStateSliderPendingTarget = target;
        clothStateSliderLastTarget = target;
        clothStateSliderSuppressCallback = true;
        try
        {
            if (clothStateSliderJSON != null)
        ...
        clothStateSliderSuppressCallback = false;
        int hiddenNowForExternal = Mathf.Clamp(GetClothStateSliderCurrentHidden(), 0, max);
        UpdateClothStateSliderExternalValues(hiddenNowForExternal, max, target);
        UpdateClothStateSliderStatus("queued", hiddenNowForExternal, max, true);

        clothStateSliderApplyVersion++;
        clothStateSliderApplyRoutine = StartCoroutine(ApplyClothStateSliderTargetRoutine(target, clothStateSliderApplyVersion));
    }

    private void StopClothSliderConflictingOperations()
    {
        if (autoModeChooser != null && autoModeChooser.val != AUTO_OFF)
            autoModeChooser.val = AUTO_OFF;
        ...
        {
            wearNoneCancelVersion++;
            DebugLog("[CLOTH SLIDER] cancel wearNoneRoutine / cancelVersion=" + wearNoneCancelVersion.ToString(CultureInfo.InvariantCulture));
            try { StopCoroutine(wearNoneRoutine); } catch { }
            wearNoneRoutine = null;
        }
        ...
        {
            // v033: slider may be running grouped PHTY via PhysicalHideGroupRoutine.
            // Bump the shared token before restoring values so any in-flight group exits cleanly.
            wearNoneCancelVersion++;
            StopActivePhysicalRoutine(true);
        }
    ...
    private IEnumerator ApplyClothStateSliderTargetRoutine(int targetHiddenCount, int version)
    {
        if (orderedClothes.Count == 0)
        {
            SetStatus("Cloth Slider skipped: no cloth list");
            clothStateSliderApplyInProgress = false;
            clothStateSliderPendingTarget = -1;
            UpdateClothStateSliderExternalValues(0, 0, 0);
            RefreshClothStateSliderRangeAndValue(true);
            UpdatePreview();
            clothStateSliderApplyRoutine = null;
            yield break;
        ...
        StopClothSliderConflictingOperations();

        int max = Mathf.Max(0, GetClothStateSliderMax());
        int target = Mathf.Clamp(targetHiddenCount, 0, max);
        int current = Mathf.Clamp(GetClothStateSliderCurrentHidden(), 0, max);
        int changed = 0;
        ...
        clothStateSliderPendingTarget = target;
        clothStateSliderLastTarget = target;
        UpdateClothStateSliderExternalValues(current, max, target);
        UpdateClothStateSliderStatus("applying", current, max, true);

        DebugLog("[CLOTH SLIDER] apply target=" + target.ToString(CultureInfo.InvariantCulture) +
        ...
        while (version == clothStateSliderApplyVersion && current < target)
        {
            // v033: For PHTY DOWN/UP/FADE, do not run one item at a time.
            // Build the required visible items and run the existing grouped PHTY routine once.
            // This makes FADE simultaneous and also avoids duration * item-count waits.
            if (UsePhysicalRemove())
            {
                int need = Mathf.Max(1, target - current);
                List<ClothItem> group = BuildClothSliderHideGroup(need);
                if (group == null || group.Count == 0)
                    break;

                ClothItem groupLast = group[group.Count - 1];
                int beforeHidden = current;
                string requestedStyle = physRemoveStyleChooser != null ? physRemoveStyleChooser.val : PHYS_OFF;

                // v037/v038: FADE from slider should not behave like a queue. Start all target items
                // together and return the slider/apply state immediately; the visual fade completes
                // in the background and then applies hideMaterial.
                if (requestedStyle == PHYS_FADE)
                {
                    int started = StartClothSliderFadeGroupBackground(group, "slider_fade");
                    current = Mathf.Clamp(GetClothStateSliderCurrentHidden(), 0, max);
                    int fadeDeltaHidden = Mathf.Max(0, current - beforeHidden);
                    if (started <= 0 || fadeDeltaHidden <= 0)
                        break;

                    changed += fadeDeltaHidden;
                    lastChanged = groupLast;
                    RefreshClothStateSliderRangeAndValue(true);
                    yield return null;
                    continue;
                }
                ...
                UpdateButtonColors();
                UpdateClothStateSliderStatus("PHTY group", current, max, true);
                yield return StartCoroutine(PhysicalHideGroupRoutine(group, "slider_group", "Cloth Slider", cancelToken));
                physicalRemoveRunning = Mathf.Max(0, physicalRemoveRunning - 1);
                UpdateButtonColors();

                if (version != clothStateSliderApplyVersion || IsWearNoneCancelled(cancelToken))
                {
                    if (version == clothStateSliderApplyVersion)
                    {
                        clothStateSliderApplyInProgress = false;
                        clothStateSliderPendingTarget = -1;
                    }
                ...
                current = Mathf.Clamp(GetClothStateSliderCurrentHidden(), 0, max);
                int groupDeltaHidden = Mathf.Max(0, current - beforeHidden);
                if (groupDeltaHidden <= 0)
                    break;

                changed += groupDeltaHidden;
                lastChanged = groupLast;
                RefreshClothStateSliderRangeAndValue(true);
                yield return null;
                continue;
            ...
            ClothItem item = FindNextClothSliderHideCandidate();
            if (item == null)
                break;
            ...
            {
                // RIPPER still runs one-by-one because ClothingRipper actions can have their own busy/queue behavior.
                ok = SliderRipperHideOneItem(item);
                if (!ok)
                {
                    StartPhysicalHideWithStyle(item, "slider", PHYS_DOWN);
                    yield return StartCoroutine(WaitForClothSliderPhysicalDone(version));
                    ok = IsHidden(item);
                }
            }
            else
            {
                ok = SliderHideOneItem(item);
            }
        ...
        while (version == clothStateSliderApplyVersion && current > target)
        {
            ClothItem item = FindNextClothSliderWearCandidate();
            if (item == null)
                break;

            bool ok = SliderWearOneItem(item);
            if (!ok)
                break;
        ...
        clothStateSliderApplyInProgress = false;
        clothStateSliderPendingTarget = -1;
        UpdateClothStateSliderExternalValues(current, max, current);

        PublishClothState("slider", lastChanged, true);
        ResetAutoTimerAfterStep();
        UpdateButtonColors();
        RefreshClothStateSliderRangeAndValue(true);
        SetStatus("Cloth Slider: " + current.ToString(CultureInfo.InvariantCulture) + "/" + max.ToString(CultureInfo.InvariantCulture) + " hidden / changed=" + changed.ToString(CultureInfo.InvariantCulture));
        UpdatePreview();
        clothStateSliderApplyRoutine = null;
    }

    private IEnumerator WaitForClothSliderPhysicalDone(int version)
    {
        while (version == clothStateSliderApplyVersion && (physicalRemoveRunning > 0 || physicalHideRoutine != null))
            yield return null;
    }

    private List<ClothItem> BuildClothSliderHideGroup(int count)
    {
        List<ClothItem> group = new List<ClothItem>();
        int need = Mathf.Max(0, count);
        if (need <= 0)
            return group;
        ...
        {
            ClothItem item = orderedClothes[i];
            if (item == null || IsProtected(item) || IsHidden(item) || IsClothSliderPendingHidden(item))
                continue;
    ...
    private bool IsClothSliderPendingHidden(ClothItem item)
    {
        return item != null && !string.IsNullOrEmpty(item.Prefix) && clothStateSliderPendingHiddenPrefixes.Contains(item.Prefix);
    }

    private int StartClothSliderFadeGroupBackground(List<ClothItem> group, string actionBase)
    {
        if (group == null || group.Count == 0)
            return 0;

        float fadeSeconds = fadeSecondsJSON != null ? Mathf.Max(0.01f, fadeSecondsJSON.val) : 1.0f;
        List<ClothItem> preparedItems = new List<ClothItem>();
        ...
        for (int i = 0; i < group.Count; i++)
        {
            ...
            if (prepFound <= 0)
            {
                DebugLog("[CLOTH SLIDER FADE SKIP] prep material refs not found: " + item.Name);
                continue;
            }

            preparedItems.Add(item);
            if (!string.IsNullOrEmpty(item.Prefix))
                clothStateSliderPendingHiddenPrefixes.Add(item.Prefix);
        ...
        SetStatus("[CLOTH SLIDER FADE START] count=" + preparedItems.Count.ToString(CultureInfo.InvariantCulture) + " / " + fadeSeconds.ToString("F2", CultureInfo.InvariantCulture) + " sec");
        PublishClothState(actionBase + "_start", null, true);
        UpdateButtonColors();
        RefreshClothStateSliderRangeAndValue(true);
        UpdatePreview();

        StartCoroutine(ClothSliderFadeGroupBackgroundRoutine(preparedItems, fadeRefs, fadeSeconds, actionBase));
        return preparedItems.Count;
    }

    private IEnumerator ClothSliderFadeGroupBackgroundRoutine(List<ClothItem> group, List<FadeParamRef> fadeRefs, float fadeSeconds, string actionBase)
    {
        if (fadeRefs == null)
            fadeRefs = new List<FadeParamRef>();
        ...
        if (group != null)
        {
            for (int i = 0; i < group.Count; i++)
            {
                ...
                if (!string.IsNullOrEmpty(item.Prefix))
                    clothStateSliderPendingHiddenPrefixes.Remove(item.Prefix);
            }
        }

        RestoreFadeSettings(fadeRefs);
        stateCacheValid = false;
        SetStatus("[CLOTH SLIDER FADE DONE] hidden=" + hiddenCount.ToString(CultureInfo.InvariantCulture));
    ...
    private ClothItem FindNextClothSliderHideCandidate()
    {
        for (int i = 0; i < orderedClothes.Count; i++)
        {
            ClothItem item = orderedClothes[i];
    ...
    private ClothItem FindNextClothSliderWearCandidate()
    {
        for (int i = orderedClothes.Count - 1; i >= 0; i--)
        {
            ClothItem item = orderedClothes[i];
            if (item == null || IsProtected(item) || !IsHidden(item))
                continue;
            return item;
        }
        return null;
    ...
    private bool SliderRipperHideOneItem(ClothItem item)
    {
        if (item == null || IsProtected(item) || IsHidden(item))
            return false;

        bool ok = CallRipperRipItemAction(item, "CLOTH SLIDER / PHTY RIPPER");
        if (!ok)
        {
            DebugLog("[CLOTH SLIDER RIPPER] rip failed, fallback DOWN: " + item.Name);
            return false;
        }

        MarkRipperSelectedHidden(item);
        SetStatus("Cloth Slider RIPPER: " + item.Name);
        PublishClothState("slider_ripper", item, true);
        UpdateButtonColors();
        UpdatePreview();
        return true;
    }

    private bool SliderHideOneItem(ClothItem item)
    {
        if (item == null || IsProtected(item) || IsHidden(item))
            return false;

        int prepFound = PrepareVisualStateForAction(item, "SLIDER_HIDE_PREP", false);
        if (prepFound <= 0)
        {
            DebugLog("[CLOTH SLIDER HIDE SKIP] prep material refs not found: " + item.Name);
            return false;
        }
        ...
        if (found <= 0)
        {
            DebugLog("[CLOTH SLIDER HIDE SKIP] stale material refs: " + item.Name);
            return false;
        }

        DebugLog("[CLOTH SLIDER HIDE] " + item.Name);
        return true;
    }

    private bool SliderWearOneItem(ClothItem item)
    {
        if (item == null || IsProtected(item) || !IsHidden(item))
            return false;
        ...
        {
            bool ripperOk = CallRipperRestoreItemAction(item, "CLOTH SLIDER / RIPPER RESTORE");
            if (ripperOk)
                ClearRipperSelectedHidden(item);
            else
                DebugLog("[CLOTH SLIDER WEAR] RIPPER restore failed, trying material restore: " + item.Name);
        }

        int found = RestoreVisibleForItemNoVisualRestore(item, "SLIDER_WEAR");
        if (found <= 0)
        {
            DebugLog("[CLOTH SLIDER WEAR SKIP] material refs not found: " + item.Name);
            return false;
        }
        ...
        DebugLog("[CLOTH SLIDER WEAR] " + item.Name);
        return true;
    }
...
TraceAlphaAdjustState(item, context + "_AFTER_SET_VISIBLE_BEFORE_NUDGE_TRACE");
    ...
    private bool UpdateClothingRipperDetection(string reason)
    {
        ...
        RefreshPhysRemoveStyleChoicesForRipper(found, reason);
    ...
    private void UpdatePreview()
    {
        ...
        text += "\n";
        text += "WEAR ALL=全部着る / WEAR NONE=全部脱ぐ / PREV=1枚着る / NEXT=1枚脱ぐ / Slider=段階指定\n";
